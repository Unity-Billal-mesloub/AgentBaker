...
node_exporter_copy_baseline_assets() {
    ...
    mkdir -p /usr/local/bin /etc/systemd/system /etc/node-exporter.d

    cp -f "${NODE_EXPORTER_ROOT_DIR}/node-exporter-startup.sh" /usr/local/bin/
    chmod +x /usr/local/bin/node-exporter-startup.sh
    ln -sf /usr/bin/node-exporter /usr/local/bin/node-exporter
    ...
    chmod 644 /etc/systemd/system/*.service /etc/systemd/system/*.path /etc/node-exporter.d/web-config.yml "${NODE_EXPORTER_SKIP_FILE}"

    systemctl daemon-reload
    systemctl enable "${NODE_EXPORTER_SERVICE_NAME}"
    systemctl enable node-exporter-restart.path
}
...
node_exporter_extract_package_version() {
    ...
    if [[ "${os_type}" == "ubuntu" ]]; then
        local version=$(jq -r --arg release "${os_release}" '.downloadURIs.ubuntu[$release].versionsV2[0].latestVersion' <<<"${package_json}")
        # Parse "1.9.1-ubuntu20.04u5" -> version:revision:ubuntu_version
        [[ "${version}" =~ ^([0-9.]+)-ubuntu([0-9.]+)u([0-9]+)$ ]]
...
installNodeExporter() {
    ...
    # Skip for OSGuard and Flatcar
    if [[ ("${OS}" == "${AZURELINUX_OS_NAME}" && "${OS_VARIANT}" == "${AZURELINUX_OSGUARD_OS_VARIANT}") || "${OS}" == "FLATCAR" ]]; then
        echo "[node-exporter] Skipping for ${OS} ${OS_VARIANT:-default}"
    ...
    local version_info ubuntu_release
    if [[ "${OS}" == "${MARINER_OS_NAME}" ]]; then
        ...
        case "${OS_VERSION}" in
            20.04) ubuntu_release="r2004" ;;
            22.04) ubuntu_release="r2204" ;;
            24.04) ubuntu_release="r2404" ;;
        esac
        version_info=$(node_exporter_extract_package_version "${package_json}" "ubuntu" "${ubuntu_release}")
    fi
