...
# Check if secure-boot.pem exists
...
if [[ -f "$CERT_PATH" ]]; then
    ...
    SIG_LOCATION=$(az sig show --ids ${GALLERY_RESOURCE_ID} --query location -o tsv)
...
BICEP_EOF
    ...
    # Deploy with Bicep (use unique deployment name per build)
    DEPLOYMENT_NAME="sig-security-profile-${BUILD_ID}-$(date +%s)"
