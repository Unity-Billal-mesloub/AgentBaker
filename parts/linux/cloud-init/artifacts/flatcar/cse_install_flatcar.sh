...
downloadSysextFromVersion() {
    ...
    local tries=0
    while ! timeout 60 oras pull --registry-config "${ORAS_REGISTRY_CONFIG_FILE}" --output "${downloadDir}" "${seURL}"; do
...
matchRemoteSysext() {
    local seURL=$1 desiredVer=$2 seArch=$3 tries=0
    while ! { timeout 20 oras repo tags --registry-config "${ORAS_REGISTRY_CONFIG_FILE}" "${seURL}" | grep -Ex "v${desiredVer//./\\.}[.~-].*-azlinux3-${seArch}" | sort -V | tail -n1; test ${PIPESTATUS[0]} -eq 0; }; do
